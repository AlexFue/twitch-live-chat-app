...
const StatusBadge: React.FC<Props> = ({ streamerInfo, currentChannel, wsStatus }: Props) => {
  if (wsStatus === 'connecting' || wsStatus === 'disconnected') {
    return (
      <div className="flex items-center gap-2 text-sm text-gray-400">
        <span className="inline-block w-2 h-2 rounded-full bg-yellow-500 animate-pulse" />
        {wsStatus === 'connecting' ? 'Connecting to server...' : 'Reconnecting...'}
      </div>
    );
  }
    ...
    return (
      <div className="flex flex-col gap-1">
        <div className="flex items-center gap-2">
          ...
          <span className="text-white font-semibold">{streamerInfo.displayName}</span>
          <span className="text-gray-400 text-sm">
            {streamerInfo.viewerCount?.toLocaleString()} viewers
          </span>
        </div>
        {streamerInfo.title && (
          <p className="text-gray-400 text-sm truncate max-w-md" title={streamerInfo.title}>
            {streamerInfo.title}
          </p>
        )}
      </div>
