...
const ChatFeed: React.FC<Props> = ({ messages, clearMessages, currentChannel }: Props) => {
  ...
  if (!currentChannel) {
    return (
      <div className="flex-1 flex flex-col items-center justify-center text-gray-500 gap-3">
        <TwitchIcon />
        <p className="text-lg">
          Enter a streamer name above to start watching chat
        </p>
      </div>
    );
  }
  ...
  return (
    <div className="flex flex-col">
      ...
      <button
        ...
        className="mt-4 mx-auto px-4 py-2 bg-red-600 hover:bg-red-500 rounded-lg text-white disabled:opacity-50 disabled:cursor-not-allowed transition-colors"
      >
        Clear Chat
      </button>
    </div>
