...
router.get('/', async (req: Request, res: Response) => {
  ...
  try {
    ...
    if (userRes.status === 401) {
      ...
      if (!retry.ok) throw new Error('Helix users request failed after token refresh');
    ...
    if (!userRes.ok) throw new Error(`Helix users request failed: ${userRes.status}`);
    ...
    const streamRes = await fetchHelix(`/helix/streams?user_login=${login}`, token);
    if (!streamRes.ok) throw new Error(`Helix streams request failed: ${streamRes.status}`);
